...
const actions = {
    ...
    [actionTypes.sockDataFn] ({commit, dispatch, state}, info) {
        if (info.mtype && info.mtype === '4') {
            ...
            case 'hot':
                let storeMatchListHot = state.matchList_hot
    ...
    [actionTypes.setIntervalFn] ({commit, dispatch, state}, params) {
        ...
        case 'hot':
            interval = setInterval(() => {
                ...
                dispatch(actionTypes.getFootballMatchList, 'run')
            }, params.time)
                ;break
        case 'run':
            interval = setInterval(() => {
    ...
    /* 比赛列表  含热门 直播 全部未结束  */
    async [actionTypes.getFootballMatchList] ({commit, dispatch, state}, listtype = 'hot') {
        try {
